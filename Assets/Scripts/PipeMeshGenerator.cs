...
public class PipeMeshGenerator : MonoBehaviour {
    ...
    void Awake()
    {
        ...
        currentMeshFilter = currentMeshFilter != null ? currentMeshFilter : gameObject.AddComponent<MeshFilter>();
        m = new Mesh();
        if (generateOnStart) {
            RenderPipe();
        }
    ...
    public void SetPoints(LineRenderer lr)
    {
    ...
    public void SetPoints(Vector3[] pts, int len)
    {
        points.Clear();
        for (int i = 0; i < len; i++)
        {
